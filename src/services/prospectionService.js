...
const CONTACT_SELECTION_CRITERIA = {
  decisionMakerRoles: [
    'CEO', 'PDG', 'President', 'Directeur Général', 
    'CTO', 'CDO', 'CIO', 'CFO', 
    'Directeur', 'Vice-Président', 'VP'
  ],
  expertiseKeywords: [
    'digital', 'transformation', 'innovation', 
    'data', 'cybersécurité', 'stratégie', 
    'analytique', 'intelligence artificielle', 
    'cloud', 'sécurité', 'performance',
    'erp', 'crm', 'marketing', 'commercial', 
    'supply chain', 'logistique', 'ressources humaines'
  ],
  hierarchyBonus: {
...
class ProspectionService {
  ...
  calculateContactRelevance(contact, opportunities) {
    ...
    const isDecisionMaker = CONTACT_SELECTION_CRITERIA.decisionMakerRoles.some(role => 
      roleLower.includes(role.toLowerCase())
    );
    totalScore += isDecisionMaker ? 0.3 : 0.1;

    // 2. Analyse de l'expertise
    const expertiseMatches = CONTACT_SELECTION_CRITERIA.expertiseKeywords.filter(keyword => 
    ...
    for (const [rolePattern, bonus] of Object.entries(CONTACT_SELECTION_CRITERIA.hierarchyBonus)) {
    ...
    opportunities.forEach(opportunity => {
      ...
      if (roleLower.includes(offerLower) || departmentLower.includes(offerLower)) {
        totalScore += 0.3;
      }

      // Correspondance avec l'actualité
      if (roleLower.includes(newsLower) || departmentLower.includes(newsLower)) {
        totalScore += 0.2;
      }
    });

    // 5. Vérification de l'email professionnel
    const isValidEmail = CONTACT_SELECTION_CRITERIA.companyDomains.some(domain => 
