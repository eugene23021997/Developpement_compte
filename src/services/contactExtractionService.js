...
const PATTERNS = {
  ...
  PERSON_WITH_TITLE:
    /([A-Z][a-zÀ-ÿ]+(?:\s[A-Z][a-zÀ-ÿ]+)+)\s*,\s*((?:[^,.]|[dD]'|[dD]e\s)+(?:directeur|directrice|président|présidente|CEO|PDG|DG|DSI|CFO|CTO|vice-président|VP|responsable|manager|chef|head|leader|dirigeant|fondateur|fondatrice|chief|officer|executive)\s+(?:[^,.])+)/gi,

  // Pour les cas comme "M. Dupont, Directeur..."
  PERSON_WITH_TITLE_MR_MRS:
    /(?:M\.|Mme|Mlle|Mr\.|Mrs\.|Ms\.)\s+([A-Z][a-zÀ-ÿ]+(?:\s[A-Z][a-zÀ-ÿ]+)*)\s*,\s*((?:[^,.]|[dD]'|[dD]e\s)+(?:directeur|directrice|président|présidente|CEO|PDG|DG|DSI|CFO|CTO|vice-président|VP|responsable|manager|chef|head|leader|dirigeant|fondateur|fondatrice|chief|officer|executive)\s+(?:[^,.])+)/gi,

  // Motifs comme "Directeur X, Jean Dupont"
  TITLE_THEN_PERSON:
    /((?:directeur|directrice|président|présidente|CEO|PDG|DG|DSI|CFO|CTO|vice-président|VP|responsable|manager|chef|head|leader|dirigeant|fondateur|fondatrice|chief|officer|executive)\s+(?:[^,.])+),\s+([A-Z][a-zÀ-ÿ]+(?:\s[A-Z][a-zÀ-ÿ]+)+)/gi,

  // Forme simple "Jean Dupont (Directeur...)"
  PERSON_TITLE_PARENTHESIS:
    /([A-Z][a-zÀ-ÿ]+(?:\s[A-Z][a-zÀ-ÿ]+)+)\s*\(([^()]*(?:directeur|directrice|président|présidente|CEO|PDG|DG|DSI|CFO|CTO|vice-président|VP|responsable|manager|chef|head|leader|dirigeant|fondateur|fondatrice|chief|officer|executive)[^()]*)\)/gi,

  // Formes comme "X a nommé Jean Dupont au poste de Directeur..."
  NAMED_AS:
    /(?:a\s+nommé|nomme|désigne|a\s+été\s+nommé|vient\s+d[e\']être\s+nommé|a\s+promu|promeut|est\s+nommé|devient|a\s+été\s+désigné)\s+([A-Z][a-zÀ-ÿ]+(?:\s[A-Z][a-zÀ-ÿ]+)+)\s+(?:au\s+poste\s+de|comme|en\s+tant\s+que|en\s+qualité\s+de)\s+((?:[^,.]|[dD]'|[dD]e\s)+(?:directeur|directrice|président|présidente|CEO|PDG|DG|DSI|CFO|CTO|vice-président|VP|responsable|manager|chef|head|leader|dirigeant|fondateur|fondatrice|chief|officer|executive)\s+(?:[^,.])+)/gi,
...
function extractContacts(text, company = "Schneider Electric") {
  ...
  while ((match = PATTERNS.POTENTIAL_NAMES.exec(cleanText)) !== null) {
    ...
    if (
      ...
      KNOWN_ENTITIES.some((entity) => name.includes(entity))
    ) {
      continue;
    }
