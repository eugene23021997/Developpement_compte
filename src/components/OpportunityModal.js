...
const OpportunityModal = ({ newsData, isOpen, onClose, contacts = [] }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = prospectionService.subscribe(handleOpportunitiesChanged);
  ...
  const findMatchingContacts = (offer) => {
    ...
    const newsKeywords = `${newsData.news} ${newsData.newsDescription || ""}`.toLowerCase().split(/[\s,&]+/);
    ...
    const relevantContacts = contacts.filter(contact => {
      ...
      significantNewsKeywords.forEach(keyword => {
        if (keyword.length > 5) { // Mots-clés significatifs seulement
          if (roleText.includes(keyword)) {
            matchLevel += 2;
      ...
      return (isDecisionMaker && (directOfferMatch || directNewsMatch)) || 
             (hasExpertise && (directOfferMatch || directNewsMatch)) ||
             (directOfferMatch && directNewsMatch && matchLevel >= 7);
    });
    
    console.log(`Nombre de contacts pertinents trouvés: ${relevantContacts.length}`);
    ...
    if (relevantContacts.length < 5) {
      const keyDecisionMakers = contacts.filter(contact => {
        ...
        const isTopExecutive = roleText.includes("président") || 
                              roleText.includes("ceo") || 
        ...
        return isTopExecutive && hasMinimalMatch && !relevantContacts.includes(contact);
      });
    ...
    const scoredContacts = relevantContacts.map(contact => {
      ...
      const scoreDetails = {
        ...
        offerMatch: significantOfferKeywords.some(k => roleText.includes(k)) ? 5 : 0,
        newsMatch: significantNewsKeywords.some(k => roleText.includes(k)) ? 3 : 0,
        expertiseBonus: getExpertiseBonusByCategory(offer.category).some(term => roleText.includes(term)) ? 3 : 0,
        roleLevel: roleText.includes("directeur") ? 4 : (roleText.includes("responsable") ? 2 : 0)
      };
    ...
    }).sort((a, b) => b.relevanceScore - a.relevanceScore).slice(0, maxContacts);
    
    console.log(`Nombre final de contacts sélectionnés: ${scoredContacts.length}`);
  ...
  const getContextualKeywords = (category, detail) => {
    ...
    const detailWords = detail.toLowerCase().split(/[\s,&-]+/).filter(word => word.length > 3);
  ...
  const getContextualKeywordsFromNews = (newsTitle, category) => {
    ...
    const titleWords = newsTitle.toLowerCase()
      .split(/[\s,.:;?!()-]+/)
      .filter(word => word.length > 5 && !["schneider", "electric"].includes(word));
  ...
  const getExpertiseBonusByCategory = (category) => {
    switch(category) {
      case "Finance & Risk":
        return ["comptable", "contrôleur", "auditeur", "fiscal", "conformité", "trésorier"];
      case "Technology":
        return ["architecte", "développeur", "cyber", "data", "réseau", "infrastructure"];
      case "Operations":
        return ["production", "approvisionnement", "maintenance", "qualité", "lean", "logistique"];
      case "People & Strategy":
        return ["talent", "organisation", "transformation", "conduite", "change", "projet"];
      case "Customer & Growth":
        return ["marketing", "commercial", "vente", "digital", "produit", "crm"];
      case "BE Capital":
        return ["fusion", "m&a", "transaction", "acquisition", "investissement", "private"];
      default:
        return [];
    }
  };
  ...
  return (
    <div className="opportunity-modal-overlay" onClick={onClose}>
          ...
          <div className="offers-detail-container">
            ...
            <div className="offers-list-section">
              ...
              <div className="offers-list">
                {newsData.offers && newsData.offers.map((offer, index) => {
                  const offerKey = `${offer.category}:${offer.detail}`;
                  ...
                  return (
                    <div 
                      key={index} 
                      className={`offer-item ${selectedOfferDetails === offer ? 'active' : ''} ${isPotential ? 'potential' : ''}`}
                      onClick={() => showOfferDetails(offer)}
                    >
                      <div className="offer-item-header">
                        <span className="offer-category">{offer.category}</span>
                        <div className={`relevance-badge relevance-${offer.relevanceScore}`}>
                      ...
                      <div className="offer-actions">
                        <button 
                          className={`offer-toggle-button ${isSelected ? 'selected' : ''}`}
                          onClick={(e) => {
                            e.stopPropagation();
              ...
              {selectedOfferDetails ? (
                <>
                  <div className="selected-offer-header">
                    ...
                    <div className={`relevance-badge relevance-${selectedOfferDetails.relevanceScore}`}>
                  ...
                  <div className="selected-offer-details">
                    ...
                    <p className="offer-category-label">Catégorie: {selectedOfferDetails.category}</p>
                    ...
                    <div style={{ marginTop: "16px", fontSize: "14px", color: "#4b5563" }}>
                      <p>Cette offre <strong>{selectedOfferDetails.detail}</strong> est pertinente par rapport à l'actualité <strong>"{newsData.news}"</strong>. 
                      {selectedOfferDetails.relevanceScore === 3 && " Cette actualité constitue une opportunité idéale pour engager une démarche commerciale sur ce sujet."}
                      {selectedOfferDetails.relevanceScore === 2 && " Cette actualité pourrait constituer une bonne entrée en matière pour aborder ce sujet."}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="recommended-contacts">
                    ...
                    {loading ? (
                      <div className="contacts-loading">
                        ...
                        <p>Analyse des contacts en fonction de l'actualité et de l'offre...</p>
                      </div>
                            ...
                            <div key={index} className="contact-card">
                              ...
                              <div className="contact-score">
                                <div className={`contact-relevance-badge ${
                                  contact.relevanceScore > 12 ? 'high' : 
                                  contact.relevanceScore > 8 ? 'medium' : 'low'
                                }`}>
                                  {Math.min(99, Math.round(contact.relevanceScore * 7))}%
                                </div>
                                
                                <div className="relevance-details">
                                  {contact.scoreDetails?.offerMatch > 0 && (
                                    <span className="relevance-tag">Match direct</span>
                                  )}
                                  {contact.scoreDetails?.expertiseBonus > 0 && (
                                    <span className="relevance-tag">Expertise</span>
                                  )}
                                  {contact.scoreDetails?.roleLevel > 3 && (
                                    <span className="relevance-tag">Décideur</span>
                                  )}
                                </div>
                              </div>
                            </div>
                      ...
                      <div className="no-contacts-message">
                        ...
                        <p>Aucun contact ne correspond précisément à cette opportunité.</p>
                ...
                <div className="no-offer-selected">
                  ...
                  <p>Sélectionnez une offre pour voir ses détails et les contacts recommandés.</p>
                </div>
              )}
            </div>
          </div>
      ...
      <style jsx>{`
        ...
        .offer-item.active {
          border-color: #0071f3;
          box-shadow: 0 0 0 1px rgba(0, 113, 243, 0.5), 0 4px 12px rgba(0, 113, 243, 0.1);
        }
        ...
        .no-contacts-message, .no-offer-selected {
          display: flex;
          flex-direction: column;
          align-items: center;
          padding: 32px 0;
        ...
        .no-contacts-message svg, .no-offer-selected svg {
          margin-bottom: 16px;
          opacity: 0.5;
        }
        
        .no-contacts-message p, .no-offer-selected p {
          margin: 0;
          font-size: 14px;
        }
